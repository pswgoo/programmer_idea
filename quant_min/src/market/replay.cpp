...
std::size_t ReplayEngine::run(const std::function<void(const Tick&)>& on_tick, q::LatencyRecorder& latency, std::size_t sample_every) {
  ...
  while (auto line = reader.next_line()) {
    ...
    // detect header
    if (n == 0 && (fields[0] == "ts_ns" || fields[1] == "side")) {
      continue;
    }

    Tick t{};
    if (!parse_i64(fields[0], t.ts_ns)) continue;
    if (!parse_side(fields[1], t.side)) continue;
    if (!parse_i64(fields[2], t.price)) continue;
    if (!parse_i64(fields[3], t.qty)) continue;

    if (first_ts < 0) first_ts = t.ts_ns;
    ...
    //   }
        std::int64_t logic_dt = t.ts_ns - first_ts;
    ...
    if (cfg_.print_every && (n % static_cast<std::size_t>(cfg_.print_interval) == 0)) {
      q::log::info("Processed ticks: " + std::to_string(n));
    }
  }
